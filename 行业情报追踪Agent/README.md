...
每天自动替你刷完行业新闻——从 6 个中英文媒体源抓取当日资讯，由 Claude 完成分类、重要性评分和摘要提炼，过滤掉低质内容，最终生成一份结构清晰的行业情报简报，让你 5 分钟内掌握当天最值得关注的动态。

### 能做什么

**多源自动抓取**：同时订阅 6 个中英文媒体 RSS（36氪、虎嗅、钛媒体、TechCrunch、The Verge、Electrek），只保留当天发布、命中自动驾驶 / 新能源 / AI 等关键词的文章，无关内容自动过滤。

**AI 三步处理每篇文章**：调用 Claude API 对每篇文章依次完成：
- **分类**：划入政策法规 / 技术突破 / 融资并购 / 产品发布 / 竞争动态五大类
- **重要性评分（1-5 分）**：低于 3 分的文章自动屏蔽，不进入结果列表
- **一句话摘要**：提炼核心信息，不用点开原文

**每日行业综述**：所有文章处理完毕后，AI 汇总生成约 150 字的趋势总结，直接告诉你今天行业的整体动向。

**可视化界面**：Streamlit 网页展示，支持按分类筛选文章、实时抓取进度条；文章按重要性评分从高到低排列，最重要的内容永远排最前。

**定时自动运行**：配置后每天 08:00 自动触发抓取和分析，打开页面即可看到当日简报，无需手动操作。
